...
<table id="member1_rboardList" class="listbox">
	<thead>
    <tr>
    ...
    <tbody>
    <c:choose>
    <c:when test="${member1_rboardListSize<=0}">
    <tr>
	...
	<c:otherwise>
	<c:forEach items="${member1_rboardList}" var="member1_rboardVO">
    <tr>
      <td><input name="m1rseq" type="checkbox" value="${member1_rboardVO.m1rseq}">
      </td>
      <td height="23" align="center" >${member1_rboardVO.m1rseq}</td>
      <td style="text-align: left; padding-left: 50px; padding-right: 0px;">   
        <a href="admin_member1_rboard_detail${pageMaker.makeQuery(pageMaker.criteria.pageNum)}&m1rseq=${member1_rboardVO.m1rseq}">
		
    	 ${member1_rboardVO.subject}     
   		</a>
   	  </td>
   	  <td>${member1_rboardVO.name} </td>
      <td><fmt:formatDate value="${member1_rboardVO.regdate}"/></td>
      <td>${member1_rboardVO.hit}</td>
